...
def clip_gradient(optimizer, grad_clip):
    for group in optimizer.param_groups:
        # print("136 group :", group)
        # print("137 group.keys() :", group.keys())

        for param in group['params']:
            # if not hasattr(param.grad, 'data'):
            #     # print("141 group :", group)
            #     print("the param :", param.size())

            param.grad.data.clamp_(-grad_clip, grad_clip)

            # # # print("139 param :", param)
...
def BoxRelationalEmbedding(f_g, dim_g=64, wave_len=1000, trignometric_embedding=True):
    ...
    # print("251 f_g.size() is: ", f_g.size())  # [10, 40, 4]
    ...
    w = (x_max - x_min) + 1.
    h = (y_max - y_min) + 1.
    # print("259 cx.size() is: ", cx.size())  # [10, 40, 1]
    # print("260 cy.size() is: ", cy.size())  # [10, 40, 1]
    # print("261 w.size() is: ", w.size())  # [10, 40, 1]
    # print("262 h.size() is: ", h.size())  # [10, 40, 1]
    # print("263 cx.view(batch_size, 1, -1).size() is: ", cx.view(batch_size, 1, -1).size())  # [10, 1, 40]

    # cx.view(1,-1) transposes the vector cx, and so dim(delta_x) = (dim(cx), dim(cx))
    delta_x = cx - cx.view(batch_size, 1, -1)
    # print("267 delta_x.size() is: ", delta_x.size())  # [10, 40, 40]
    delta_x = torch.clamp(torch.abs(delta_x / w), min=1e-3)
    # print("269 delta_x.size() is: ", delta_x.size())  # [10, 40, 40]
    delta_x = torch.log(delta_x)
    # print("271 delta_x.size() is: ", delta_x.size())  # [10, 40, 40]
    ...
    # print("281 matrix_size is: ", matrix_size)  # [10, 40, 40]
    delta_x = delta_x.view(batch_size, matrix_size[1], matrix_size[2], 1)
    delta_y = delta_y.view(batch_size, matrix_size[1], matrix_size[2], 1)
    delta_w = delta_w.view(batch_size, matrix_size[1], matrix_size[2], 1)
    delta_h = delta_h.view(batch_size, matrix_size[1], matrix_size[2], 1)
    # print("286 delta_x.size() is: ", delta_x.size())  # [10, 40, 40, 1]

    position_mat = torch.cat((delta_x, delta_y, delta_w, delta_h), -1)
    # print("289 position_mat.size() is: ", position_mat.size())  # [10, 40, 40, 4]

    if trignometric_embedding == True:
        feat_range = torch.arange(dim_g / 8).cuda()
        # print("293 feat_range is: ", feat_range)  # tensor([0., 1., 2., 3., 4., 5., 6., 7.], device='cuda:0')
        dim_mat = feat_range / (dim_g / 8)
        dim_mat = 1. / (torch.pow(wave_len, dim_mat))
        # print("296 dim_mat.size() is: ", dim_mat.size())  # torch.Size([8])

        dim_mat = dim_mat.view(1, 1, 1, -1)
        # print("299 dim_mat.size() is: ", dim_mat.size())  # [1, 1, 1, 8]
        position_mat = position_mat.view(batch_size, matrix_size[1], matrix_size[2], 4, -1)
        # print("301 position_mat.size() is: ", position_mat.size())  # [10, 40, 40, 4, 1]
        position_mat = 100. * position_mat

        mul_mat = position_mat * dim_mat
        # print("305 mul_mat.size() is: ", mul_mat.size())  # [10, 40, 40, 4, 8]
        mul_mat = mul_mat.view(batch_size, matrix_size[1], matrix_size[2], -1)
        # print("307 mul_mat.size() is: ", mul_mat.size())  # [10, 40, 40, 32]
        sin_mat = torch.sin(mul_mat)
        cos_mat = torch.cos(mul_mat)
        embedding = torch.cat((sin_mat, cos_mat), -1)
        # print("311 embedding.size() is: ", embedding.size())  # [10, 40, 40, 64]
    else:
