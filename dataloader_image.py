...
class DataLoader(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        # building image preprocessing cell
        self.IMAGENET_DEFAULT_MEAN = (0.485, 0.456, 0.406)
        self.IMAGENET_DEFAULT_STD = (0.229, 0.224, 0.225)
        self.transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(self.IMAGENET_DEFAULT_MEAN,
                                                                                         self.IMAGENET_DEFAULT_STD)])
    ...
    def get_batch(self, split, batch_size=None, seq_per_img=None):
        ...
        image_batch = []  # np.ndarray((batch_size * seq_per_img, self.opt.fc_feat_size), dtype = 'float32')
        ...
        for i in range(batch_size):
            # fetch image
            tmp_image, ix, tmp_wrapped = self._prefetch_process[split].get()
        ...
        data = {}
        image_batch, label_batch, gts, infos = \
            zip(*sorted(zip(image_batch, np.vsplit(label_batch, batch_size), gts, infos), key=lambda x: 0, reverse=True))

        data['input_images'] = np.stack(reduce(lambda x, y: x+y, [[_]*seq_per_img for _ in image_batch]))
        # data['input_images'] = image_batch
    ...
    def __getitem__(self, index):
        ...
        image = Image.open(os.path.join(self.opt.input_images, image_path)).convert('RGB')
        if self.transform is not None:
            # print("transforming by imagenet means normalization !")
...
class BlobFetcher():
    ...
    def reset(self):
        ...
        """
        # batch_size is 1, the merge is done in DataLoader class; DATA_LOADER.DROP_LAST = True
        self.split_loader = iter(data.DataLoader(
            dataset=self.dataloader, batch_size=1,
            sampler=SubsetSampler(self.dataloader.split_ix[self.split][self.dataloader.iterators[self.split]:]),
            shuffle=False, pin_memory=True, num_workers=8, prefetch_factor=1, persistent_workers=True,
...
if __name__ == "__main__":
    ...
    # loader = DataLoader(opt)  # 加载captions、appearance features、geometry features，以及psp features（待加）
    loader = DataLoaderX(opt)  # 加载images、captions、seq_mask
    ...
    # print("dataloader time cost :", time_start - time.time())
    # print("data.keys() are :", data.keys())
    #
    # print("data['input_images'].size() are :", np.shape(data["input_images"]))
    #
    # print("data['labels_masks'].size() are :", np.shape(data["labels_masks"]))
    # print("data['labels_masks'] are :", data["labels_masks"])
    #
    # print("data['gts'][0].size() are :", np.shape(data["gts"][0]))
    # print("data['gts'][0] are :", data["gts"][0])
    #
    # print("data['gts'][1].size() are :", np.shape(data["gts"][1]))
    # print("data['gts'][1] are :", data["gts"][1])
    #
    # print("data['bounds'].size() are :", np.shape(data["bounds"]))
    # print("data['bounds'] are :", data["bounds"])
    #
    # print("data['infos'] are :", data["infos"])
    # print("data are :", data)
